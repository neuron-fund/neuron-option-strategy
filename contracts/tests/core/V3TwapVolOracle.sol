...
contract V3TwapVolOracle is VolOracle {
     ...
     */
    function getPrice(address pool) public view override returns (uint256 price) {
        (int56 oldestTickCumulative, int56 newestTickCumulative, uint32 duration) = getTickCumulatives(pool);
        ...
        int24 timeWeightedAverageTick = getTimeWeightedAverageTick(
        ...
        uint256 baseCurrencyDecimals = IERC20DetailedLegacy(token0).decimals();
        uint128 quoteAmount = uint128(1 * 10**baseCurrencyDecimals);

        return OracleLibrary.getQuoteAtTick(timeWeightedAverageTick, quoteAmount, token0, token1);
    }
    ...
    function getTimeWeightedAverageTick(
        ...
        if (tickCumulativesDelta < 0 && (tickCumulativesDelta % duration != 0)) _timeWeightedAverageTick--;
        ...
        )
    {
        ...
        (, , uint16 newestIndex, uint16 observationCardinality, , , ) = uniPool.slot0();

        // Get the latest observation
        (uint32 newestTimestamp, int56 _newestTickCumulative, , ) = uniPool.observations(newestIndex);

        // Get the oldest observation
        uint256 oldestIndex = (newestIndex + 1) % observationCardinality;
        (uint32 oldestTimestamp, int56 _oldestTickCumulative, , ) = uniPool.observations(oldestIndex);
