...
library Welford {
    ...
    function update(
        ...
        if (curCount == 1 && oldValue == 0) {
            ...
            int256 meanIncrement = (newValue.sub(curMean)).div(int256(curCount));
            ...
            mean = curMean.add(meanIncrement);
            dsq = curDSQ.add((newValue.sub(oldValue)).mul(newValue.add(oldValue).sub(mean).sub(curMean)));
        }
     ...
     */
    function variance(uint256 count, int256 dsq) internal pure returns (uint256) {
